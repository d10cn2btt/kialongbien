	...
	public function get_html(){
		...
		if($this->is_enabled()){
			if($this->get_title_position() === 'left'){
				$html .= '<table class="variations_x '. $this->get_cssclass_str() .'" cellspacing="0"><tbody><tr>';
				$html .= '<td class="label_x"><label for="'. $this->get_name() .'" class="'. $this->get_title_class_str() .'">'.$this->get_title().'</label></td>';
				$html .= '<td class="value_x">';
				$html .= '<select id="'.$this->get_name().'" name="'.$this->get_name().'" placeholder="'.$this->get_placeholder().'" value="'.$this->get_value().'" >';
				foreach($this->get_options() as $option_key => $option_text){		
					$selected = ($option_text === $this->get_value()) ? 'selected' : '';		
					$html .= '<option value="'.$option_text.'" '.$selected.'>'.$option_text.'</option>';
				}
				$html .= '</select></td>';
				$html .= '</tr></tbody></table>';
			}else{
				$html .= '<table class="variations '. $this->get_cssclass_str() .'" cellspacing="0"><tbody><tr>';
